...
export default function AdminDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="mb-8">
        <p className="text-yellow-600 text-xs uppercase tracking-widest mb-1">Admin Area</p>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-10">
        ...
        <Link href="/products"
          className="group bg-white border border-gray-200 hover:border-blue-400 rounded-xl p-6 flex items-center justify-between transition-all shadow-sm">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center">
              <ShoppingBag size={24} className="text-blue-500" />
            </div>
            <div>
              <p className="font-semibold text-gray-900">View Storefront</p>
      ...
      {/* Product list */}
      <h2 className="font-heading text-xl font-bold mb-4 text-gray-900">All Products</h2>
              ...
              {products.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-yellow-600 font-medium">{formatPrice(price)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${p.is_active ? 'bg-green-50 border border-green-200 text-green-600' : 'bg-gray-100 border border-gray-200 text-gray-400'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/products/edit/${p.id}`}
                          className="inline-flex items-center gap-1.5 text-xs text-gray-500 hover:text-yellow-600 transition-colors border border-gray-200 hover:border-yellow-400 rounded-lg px-3 py-1.5">
                          <Pencil size={12} /> Edit
                        </Link>
                        <button
                          onClick={() => handleDelete(p.id, p.name)}
                          disabled={deletingId === p.id}
                          className="inline-flex items-center gap-1.5 text-xs text-gray-500 hover:text-red-500 transition-colors border border-gray-200 hover:border-red-300 rounded-lg px-3 py-1.5 disabled:opacity-40">
