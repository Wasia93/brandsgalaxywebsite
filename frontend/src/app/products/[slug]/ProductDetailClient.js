...
import { ShoppingCart, Star, Package, Tag, ChevronLeft, ChevronRight, Heart, Share2, Check } from 'lucide-react';
...
export default function ProductDetailClient({ initialProduct }) {
  ...
  const [selectedVariant, setSelectedVariant] = useState(() => {
    const variants = parseVariants(initialProduct?.extra_data);
  ...
  useEffect(() => {
    ...
    productsAPI.getOne(slug)
      ...
      .catch(() => { if (!initialProduct) router.push('/products'); })
